...
- No `tokio::time::sleep` to wait for spawned work: hold the `JoinHandle` (or a oneshot) and await it; a harness's bounded retry (the container ping) is fine. `start_paused` is for pure timer logic with no real I/O.
- Assert the status of every response before reading its body, follow-up reads included: a 500 otherwise surfaces as a field mismatch.
...
One HTTP request exercises routing, extractors, validation, serialisation, error mapping and the query at once; a handler unit test proves little. `sea_orm::MockDatabase` asserts on the SQL emitted today, so it fails on every refactor and passes on every schema mistake.
...
- **Name the outcome**: `creates_user_returns_201`, `duplicate_email_returns_409`. Arrange, act, assert, one blank line between them.
...
Integration tests reach only `pub` items of a **library** target, so a binary-only crate cannot be integration-tested. Every `.rs` file directly in `tests/` becomes its own test binary, so a `tests/common.rs` shows up as a spurious empty target. Each test file is one more link step, and link time dominates test builds: three to five files per service. nextest forks per test, not per binary, so parallelism is unaffected.
...
`test_app()` creates a fresh database and runs the migrations, pins the clock to `FixedClock(2026-09-13T12:00:00Z)` so `created_at` is assertable, and starts a `MockServer`: give its `base_url()` to whatever setting the code under test reads for an upstream. Two dependencies swapped, one stub started, nothing global. For how the database is provisioned see `sea-orm-postgres`. A project without `test_app()` copies `tests/common/mod.rs` from the rust-scaffolding template, then adapts `AppState` and the settings map to its own.

Do **not** add an `Arc<dyn Trait>` to `AppState` to make something swappable: Postgres and outbound HTTP have real local stand-ins.
...
The denominator leaves out four things: `entities/` is generated by `sea-orm-cli`; `migration/` is DDL every test already runs, so it only inflates the number; `main.rs` is process wiring no test calls; `telemetry.rs` is exporter setup that only executes against a live collector. The Makefile `cov` recipe and the CI job each carry this regex: change both together (`rust-tooling`). 80% is a regression alarm, not a target.
...
| `tokio::time::sleep` to wait for background work | await the `JoinHandle` or a oneshot; `start_paused` auto-advances past every timeout on the way to Postgres, so `test_app()` fails at once |
| Assert on `Utc::now()` output | inject the `Clock`; `Utc::now()` has no hook a test can intercept |
| `std::env::set_var` to configure a test | build settings from a map; `set_var` is `unsafe` under a forbid lint |
| Create `tests/common.rs` | `tests/common/mod.rs`; a file directly in `tests/` is its own test binary |
